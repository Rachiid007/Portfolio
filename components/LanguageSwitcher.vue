...
import flag_fr from '~/assets/flags/FR.svg?raw'
import flag_en from '~/assets/flags/EN.svg?raw'
...
const languages = computed(() => {
  ...
  for (const key in availableLanguage) {
    ...
    if (key == locale.value) {
      ...
      myObj.active = {  name: availableLanguage[ key ].name, flag: availableLanguage[ key ].flag }
    }
    else {
      // create an object who has the key not active
      myObj.notActive = { name: availableLanguage[ key ].name, flag: availableLanguage[ key ].flag }
    }
  }
  return myObj
})
...
<template>
  <div id="container" ref="target" :class="{ 'visible' :!isVisible }">
    ...
    <button class="item choice" :class="{ hidden :isVisible }" @click="handleSwitchLanguage(languages.notActive.name)">
...
.trigger {
  background-color: hsla(0,0%,99.2%,.1);
  border-radius: 8px;
  justify-content: space-around;
}
